...
class Picture:
    ...
    def __init__(self, name, space_hold, tape_hold, carpet_hold, ground_hold, pointPlus, screenshot, points):
        self.name = name
                ...
                for ground in range(0, 4):
                    if os.path.exists(f'img/MatchThree/{name}_t{tape}c{carpet}g{ground}.jpg'):
                        ...
                        for (x, y, w, h) in rectangles:
                            # cv.rectangle(self.screenshot, (x, y), (x + w, y + h), 1)
                            center_x = x + int(w / 2)
                            center_y = y + int(h / 2)
