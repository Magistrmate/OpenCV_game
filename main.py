    ...
    if len(points) > 0:
        ...
        try:
            ...
            for point in points:
                ...
                if pointName != 'canister' and point[1][2] == 0:
                    checkNames = ['cube', 'envelope', 'bomb', 'h_rocket', 'v_rocket']
                    for (stage, floor) in ((80, 19), (114, 21)):
                        if point[4][floor + 26] != 'X' and point[4][floor + 26][2] == 0:
                            if pointName not in checkNames:
                                left = 0
                                right = 0
                                up_down = 0
                                for (a, b) in zip(range(stage, stage - 31, -3), range(1, 8)):
                                    if point[4][a][0] == pointName and up_down == b - 1:
                                        up_down = b
                                    else:
                                        break
                                if point[4][floor][0] == pointName:
                                    left = 1
                                    for (a, b) in zip(range(floor - 3, floor - 19, -3), range(2, 8)):
                                        if point[4][a][0] == pointName and left == b - 1:
                                            left = b
                                        else:
                                            break
                                if point[4][floor + 22][0] == pointName:
                                    right = 1
                                    for (a, b) in zip(range(floor + 19, floor + 3, -3), range(2, 8)):
                                        if point[4][a][0] == pointName and right == b - 1:
                                            right = b
                                        else:
                                            break
                        ...
                        if point[4][side + 3] != 'X' and point[4][side + 3][2] == 0:
                            if pointName not in checkNames:
                                ...
                                for (a, b) in zip(range(side, side - 16, -3), range(1, 8)):
                                    if point[4][a][0] == pointName and left_right == b - 1:
                                        left_right = b
                                    else:
                                        break
                                if point[4][side + 2][0] == pointName:
                                    ...
                                    for (a, b) in zip(range(see, see - 31, -3), range(2, 8)):
                                        if point[4][a][0] == pointName and up == b - 1:
                                            up = b
                                        else:
                                            break
                                if point[4][side + 4][0] == pointName:
                                    ...
                                    for (a, b) in zip(range(see + 34, see + 5, -3), range(2, 8)):
                                        if point[4][a][0] == pointName and down == b - 1:
                                            down = b
                                        else:
                                            break
